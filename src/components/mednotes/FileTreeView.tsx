'use client';

import React, { useState, useCallback, useRef, useEffect } from 'react';
import FileTreeItem from './FileTreeItem';
import type { FileNode } from '@/types';
import { SidebarMenu } from '@/components/ui/sidebar';
import { ScrollArea } from '@/components/ui/scroll-area';
import { GripVertical, ClipboardType, ListChecks, Baby, BriefcaseMedical, Stethoscope, Scissors, Siren, UserCheck, UserPlus, HeartPulse, Syringe, Ambulance, BookOpenCheck, BookOpen, UserCog, User, UserSquare, UserCircle, User2, UserPlus2, UserCheck2, UserRoundCheck, UserRoundPlus, UserRound, UserRoundCog, UserRoundSearch, UserRoundX, UserRoundMinus, X, RotateCcw } from 'lucide-react';
import { Button } from '@/components/ui/button';
import * as ReactDOM from 'react-dom';
import { AnimatePresence, Reorder, motion, useDragControls } from 'framer-motion';

interface SearchBarProps {
  placeholder?: string;
  onSearch: (term: string) => void;
}

const SearchBar: React.FC<SearchBarProps> = ({ placeholder, onSearch }) => {
  const [value, setValue] = useState('');
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const newValue = e.target.value;
    setValue(newValue);
    onSearch(newValue);
  };
  return (
    <div className="relative w-full">
      <input
        type="text"
        value={value}
        onChange={handleChange}
        placeholder={placeholder}
        className="w-full px-3 py-2 mb-2 text-sm border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
      />
    </div>
  );
};

interface FileTreeViewProps {
  nodes: FileNode[];
  onFileSelect: (file: FileNode) => void;
  selectedFileId?: string;
  searchTerm: string;
  expandedFolders: Set<string>;
  toggleFolder: (folderId: string) => void;
}

const LOCAL_STORAGE_KEY = 'mednotes-subfolder-order';

const getSubfolderIcon = (parentName: string, subfolderName: string) => {
  const pName = parentName.toLowerCase();
  const sName = subfolderName.toLowerCase();
  const isKlin = sName.includes('клиническ') || sName.includes('клініч');
  const isPrakt = sName.includes('практическ') || sName.includes('практич');
  
  if (pName.includes('педіатр') || pName.includes('педиатр')) {
    return isKlin ? Baby : BookOpenCheck;
  } else if (pName.includes('акушер')) {
    return isKlin ? BriefcaseMedical : Syringe;
  } else if (pName.includes('внутрішня') || pName.includes('внутренняя')) {
    return isKlin ? Stethoscope : HeartPulse;
  } else if (pName.includes('хирург')) {
    return isKlin ? Scissors : UserCheck;
  } else if (pName.includes('екстрен') || pName.includes('экстрен')) {
    return isKlin ? Siren : Ambulance;
  }
  return ClipboardType;
};

const getAllSubfolders = (nodes: FileNode[]) => {
  return nodes.flatMap(folder =>
    (folder.children || []).filter(child => child.type === 'folder').map(sub => ({
      id: sub.id,
      name: `${folder.name} — ${sub.name}`,
      node: sub,
      parent: folder,
    }))
  );
};

type SubfolderEntry = ReturnType<typeof getAllSubfolders>[number];

interface ReorderableSubfolderItemProps {
  sf: SubfolderEntry;
  selectedFileId?: string;
  isExpanded: boolean;
  onToggle: (id: string) => void;
  onFileSelect: (file: FileNode) => void;
}

const ReorderableSubfolderItem: React.FC<ReorderableSubfolderItemProps> = ({
  sf,
  selectedFileId,
  isExpanded,
  onToggle,
  onFileSelect,
}) => {
  const dragControls = useDragControls();
  const isKlin = sf.node.name.toLowerCase().includes('клиническ');
  const isPrakt = sf.node.name.toLowerCase().includes('практическ');
  const IconComponent = getSubfolderIcon(sf.parent.name, sf.node.name);

  return (
    <Reorder.Item
      value={sf.id}
      dragListener={false}
      dragControls={dragControls}
      className="bg-sidebar-accent/60 border border-sidebar-border rounded-md mb-0.5 mt-1 p-0.5 transition-colors"
    >
      <div className="flex items-center group">
        <div
          className="flex-1 font-medium flex items-center gap-0 cursor-pointer select-none text-xs"
          onClick={() => onToggle(sf.id)}
        >
          <span
            className={
              `inline-flex items-center justify-center rounded-full mr-1 h-5 w-5 ` +
              (isKlin
                ? 'bg-blue-200 text-blue-800'
                : isPrakt
                  ? 'bg-green-200 text-green-800'
                  : 'bg-gray-200 text-gray-700')
            }
          >
            <IconComponent className="h-3.5 w-3.5" />
          </span>
          <span>{sf.name}</span>
          <motion.span
            className="ml-1 text-[10px] inline-flex origin-center"
            animate={{ rotate: isExpanded ? 90 : 0 }}
            transition={{ duration: 0.16 }}
          >
            ▶
          </motion.span>
        </div>
        <span
          className="ml-2 text-gray-400 opacity-70 group-hover:opacity-100 transition-opacity cursor-grab active:cursor-grabbing touch-none"
          title="Перетащить станцию"
          onPointerDown={(e) => {
            e.preventDefault();
            e.stopPropagation();
            dragControls.start(e);
          }}
        >
          <GripVertical />
        </span>
      </div>
      <AnimatePresence initial={false}>
        {isExpanded && (
          <motion.div
            key={`${sf.id}-files`}
            initial={{ height: 0, opacity: 0 }}
            animate={{ height: 'auto', opacity: 1 }}
            exit={{ height: 0, opacity: 0 }}
            transition={{ duration: 0.18, ease: 'easeOut' }}
            className="overflow-hidden"
          >
            <div className="ml-3 mb-1 pt-1">
              {(sf.node.children || []).filter((f: FileNode) => f.type === 'file').map((file: FileNode) => (
                <div
                  key={file.id}
                  id={`file-item-${file.id}`}
                  className={`py-0.5 px-1 rounded cursor-pointer hover:bg-blue-50 text-xs transition-colors duration-300 ${selectedFileId === file.id ? 'bg-blue-100 font-bold border-l-2 border-blue-500 shadow-sm' : ''}`}
                  onClick={e => {
                    e.stopPropagation();
                    onFileSelect(file);
                  }}
                >
                  {file.name}
                </div>
              ))}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
    </Reorder.Item>
  );
};

const FileTreeView: React.FC<FileTreeViewProps> = ({
  nodes,
  onFileSelect,
  selectedFileId,
  searchTerm,
  expandedFolders,
  toggleFolder,
}) => {
  const [searchResults, setSearchResults] = useState<{ path: string; preview: string }[]>([]);
  const [showDropdown, setShowDropdown] = useState(false);
  const dropdownRef = useRef<HTMLDivElement>(null);

  const [subfolderOrder, setSubfolderOrder] = React.useState<string[]>([]);

  const [selectMode, setSelectMode] = React.useState(false);
  const [selectedStationIds, setSelectedStationIds] = React.useState<Record<string, string | null>>({});

  const [showHelp, setShowHelp] = React.useState(false);

  const [expandedSubfolders, setExpandedSubfolders] = useState<Set<string>>(new Set());

  const [expandedUnknownFolders, setExpandedUnknownFolders] = useState<Set<string>>(new Set());

  const LOCAL_STORAGE_SELECTED_KEY = 'mednotes-selected-stations';

  useEffect(() => {
    const saved = localStorage.getItem(LOCAL_STORAGE_SELECTED_KEY);
    if (saved) {
      try {
        setSelectedStationIds(JSON.parse(saved));
      } catch (e) {
        console.error('Failed to parse selected stations', e);
      }
    }
  }, []);

  const rootFiles = nodes.filter(n => n.type === 'file');

  useEffect(() => {
    const allSubfolders = getAllSubfolders(nodes);
    const saved = localStorage.getItem(LOCAL_STORAGE_KEY);
    if (saved) {
      const savedOrder = JSON.parse(saved);
      const filtered = savedOrder.filter((id: string) => allSubfolders.some(sf => sf.id === id));
      const missing = allSubfolders.filter(sf => !filtered.includes(sf.id)).map(sf => sf.id);
      setSubfolderOrder([...filtered, ...missing]);
    } else {
      setSubfolderOrder(allSubfolders.map(sf => sf.id));
    }
  }, [nodes]);

  useEffect(() => {
    if (subfolderOrder.length) {
      localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(subfolderOrder));
    }
  }, [subfolderOrder]);

  const orderedSubfolders = React.useMemo(() => {
    const allSubfolders = getAllSubfolders(nodes);
    const idToSubfolder: Record<string, ReturnType<typeof getAllSubfolders>[number]> = Object.fromEntries(allSubfolders.map(sf => [sf.id, sf]));
    return subfolderOrder.map((id: string) => idToSubfolder[id]).filter(Boolean);
  }, [nodes, subfolderOrder]);

  useEffect(() => {
    if (selectedFileId) {
      for (const sf of orderedSubfolders) {
        if ((sf.node.children || []).some((f: FileNode) => f.id === selectedFileId)) {
          setExpandedSubfolders(prev => new Set(prev).add(sf.id));
          break;
        }
      }
    }
  }, [selectedFileId, orderedSubfolders]);

  useEffect(() => {
    if (selectedFileId) {
      setTimeout(() => {
        const selectedElement = document.getElementById(`file-item-${selectedFileId}`);
        if (selectedElement) {
          selectedElement.scrollIntoView({ behavior: 'smooth', block: 'center' });
        }
      }, 100);
    }
  }, [selectedFileId, expandedSubfolders]);

  const selectedStations = React.useMemo(() => {
    const map: Record<string, FileNode | null> = {};
    for (const sf of orderedSubfolders) {
      if (!sf) continue;
      const savedId = selectedStationIds[sf.node.id];
      if (savedId) {
        const found = (sf.node.children || []).find(c => c.id === savedId);
        if (found) {
          map[sf.node.id] = found;
          continue;
        }
      }
      map[sf.node.id] = null;
    }
    return map;
  }, [orderedSubfolders, selectedStationIds]);

  const handleSelectStationFile = (subfolderId: string, fileId: string | null) => {
    setSelectedStationIds(prev => {
      const next = { ...prev, [subfolderId]: fileId };
      localStorage.setItem(LOCAL_STORAGE_SELECTED_KEY, JSON.stringify(next));
      return next;
    });
  };

  const handleResetAllStations = () => {
    setSelectedStationIds({});
    localStorage.removeItem(LOCAL_STORAGE_SELECTED_KEY);
  };

  const toggleSubfolder = (id: string) => {
    setExpandedSubfolders(prev => {
      const next = new Set(prev);
      if (next.has(id)) next.delete(id);
      else next.add(id);
      return next;
    });
  };

  const toggleUnknownFolder = (id: string) => {
    setExpandedUnknownFolders(prev => {
      const next = new Set(prev);
      if (next.has(id)) next.delete(id);
      else next.add(id);
      return next;
    });
  };

  return (
    <ScrollArea className="h-full flex-1">
      <SidebarMenu className="p-2 relative w-80">
        <Button
          variant="outline"
          className="w-full mb-2 help-btn"
          onClick={() => setShowHelp(true)}
        >
          Как работать со станциями?
        </Button>
        {showHelp && typeof document !== 'undefined' && ReactDOM.createPortal(
          <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 p-4" onClick={() => setShowHelp(false)}>
            <div className="bg-white rounded-lg shadow-lg w-full max-w-lg flex flex-col relative max-h-[90vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>

              {/* Sticky Header with Close Button */}
              <div className="sticky top-0 z-10 flex items-center justify-between border-b px-6 py-4 bg-white">
                <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Как работать со станциями?</h2>
                <button
                  type="button"
                  className="rounded-md p-1 -mr-2 text-2xl leading-none text-gray-400 hover:bg-gray-100 hover:text-gray-700 transition"
                  onClick={() => setShowHelp(false)}
                  aria-label="Закрыть"
                >×</button>
              </div>

              {/* Scrollable Content */}
              <div className="overflow-y-auto flex-1 px-6 py-5">
                <ul className="list-disc pl-5 space-y-3 text-sm sm:text-base text-gray-700 text-justify">
                  <li><span role="img" aria-label="drag">🖱️</span> <b>Перемещение станций:</b> Теперь можно менять порядок всех клинических и практических станций независимо друг от друга. Просто тяните нужную станцию за иконку справа от названия.</li>
                  <li><span role="img" aria-label="target">🎯</span> <b>Режим "Я знаю, какие будут станции":</b> Нажмите соответствующую кнопку в меню, чтобы выбрать по одной или несколько задач из каждой подпапки (клиническая и практическая для каждого раздела). После выбора вы сможете быстро переключаться только между этими задачами.</li>
                  <li><span role="img" aria-label="lightbulb">💡</span> <b>Зачем это нужно?</b> Такой режим будет полезен, если вам уже известно, какие задачи будут ждать вас на экзамене. Выберите задачи и фокусируйтесь только на них, чтобы сэкономить время. При этом глобальный поиск по-прежнему работает по всем файлам банка.</li>
                </ul>
              </div>

              {/* Sticky Footer */}
              <div className="flex shrink-0 justify-end border-t bg-gray-50 px-6 py-4">
                <Button variant="secondary" onClick={() => setShowHelp(false)}>Понятно</Button>
              </div>
            </div>
          </div>,
          document.body
        )}
        <Button
          variant={selectMode ? 'secondary' : 'outline'}
          className="w-full mb-2"
          onClick={() => setSelectMode(m => !m)}
        >
          {selectMode ? 'Выйти из режима выбора станций' : 'Я знаю, какие будут станции'}
        </Button>
        {rootFiles.length > 0 && (
          <div className="mb-1">
            <div className="text-[11px] text-gray-500 mb-0.5">Отдельные файлы</div>
            {rootFiles.map(file => (
              <div
                key={file.id}
                id={`file-item-${file.id}`}
                className={`flex items-center px-2 py-1 rounded cursor-pointer hover:bg-blue-50 text-xs transition-colors duration-300 ${selectedFileId === file.id ? 'bg-blue-100 font-bold border-l-2 border-blue-500 shadow-sm' : ''}`}
                onClick={() => onFileSelect(file)}
              >
                <span className="mr-2">{file.icon && React.createElement(file.icon, { size: 14 })}</span>
                <span>{file.name}</span>
              </div>
            ))}
            <hr className="my-1 border-gray-300" />
          </div>
        )}
        <div className="text-[11px] text-gray-500 mb-0.5">Станции</div>
        {!selectMode && (
          <Reorder.Group axis="y" values={subfolderOrder} onReorder={setSubfolderOrder} className="list-none m-0 p-0">
            {orderedSubfolders.map((sf) => (
              <ReorderableSubfolderItem
                key={sf.id}
                sf={sf}
                selectedFileId={selectedFileId}
                isExpanded={expandedSubfolders.has(sf.id)}
                onToggle={toggleSubfolder}
                onFileSelect={onFileSelect}
              />
            ))}
          </Reorder.Group>
        )}
        {selectMode && (
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-semibold text-gray-500">Выбор задач по станциям</span>
              {Object.values(selectedStationIds).some(v => v !== null) && (
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={handleResetAllStations}
                  className="text-xs h-7 px-2 text-red-500 hover:text-red-600 hover:bg-red-50 flex items-center gap-1"
                >
                  <RotateCcw className="h-3 w-3" />
                  Сбросить всё
                </Button>
              )}
            </div>
            {orderedSubfolders.map(({ parent, node }) => {
              const file = selectedStations[node.id];
              const IconComponent = getSubfolderIcon(parent.name, node.name);
              const isKlin = node.name.toLowerCase().includes('клиническ');
              const isPrakt = node.name.toLowerCase().includes('практическ');

              if (file) {
                return (
                  <div
                    key={node.id}
                    className="border border-sidebar-border bg-sidebar-accent/30 rounded-md p-2 transition-all hover:border-blue-300"
                  >
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-[10px] uppercase font-bold tracking-wider text-gray-400 flex items-center gap-1">
                        <span
                          className={
                            `inline-flex items-center justify-center rounded-full h-4 w-4 ` +
                            (isKlin
                              ? 'bg-blue-100 text-blue-700'
                              : isPrakt
                                ? 'bg-green-100 text-green-700'
                                : 'bg-gray-100 text-gray-600')
                          }
                        >
                          <IconComponent className="h-2.5 w-2.5" />
                        </span>
                        {parent.name} — {node.name}
                      </span>
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-4 w-4 rounded-full text-gray-400 hover:text-red-500 hover:bg-gray-100"
                        onClick={(e) => {
                          e.stopPropagation();
                          handleSelectStationFile(node.id, null);
                        }}
                        title="Сбросить выбор"
                      >
                        <X className="h-3 w-3" />
                      </Button>
                    </div>
                    <div
                      className={`p-2 rounded cursor-pointer text-xs font-semibold shadow-sm transition-all border ${
                        selectedFileId === file.id
                          ? 'bg-blue-100 border-blue-500 text-blue-900 font-bold'
                          : 'bg-white border-gray-200 hover:bg-blue-50 text-gray-700'
                      }`}
                      onClick={() => onFileSelect(file)}
                    >
                      {file.name}
                    </div>
                  </div>
                );
              } else {
                const isExpanded = expandedUnknownFolders.has(node.id);
                return (
                  <div
                    key={node.id}
                    className="border border-sidebar-border bg-sidebar-accent/10 rounded-md transition-all hover:bg-sidebar-accent/25"
                  >
                    <div
                      className="p-2 flex items-center justify-between cursor-pointer select-none text-xs"
                      onClick={() => toggleUnknownFolder(node.id)}
                    >
                      <div className="flex items-center gap-1.5 font-medium text-gray-700">
                        <span
                          className={
                            `inline-flex items-center justify-center rounded-full h-5 w-5 ` +
                            (isKlin
                              ? 'bg-blue-100 text-blue-700'
                              : isPrakt
                                ? 'bg-green-100 text-green-700'
                                : 'bg-gray-100 text-gray-600')
                          }
                        >
                          <IconComponent className="h-3 w-3" />
                        </span>
                        <span>{parent.name} — {node.name}</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <span className="text-[10px] text-gray-400 italic">выберите задачу</span>
                        <motion.span
                          className="text-[10px] text-gray-400 inline-flex origin-center"
                          animate={{ rotate: isExpanded ? 90 : 0 }}
                          transition={{ duration: 0.16 }}
                        >
                          ▶
                        </motion.span>
                      </div>
                    </div>
                    <AnimatePresence initial={false}>
                      {isExpanded && (
                        <motion.div
                          key={`${node.id}-unknown-files`}
                          initial={{ height: 0, opacity: 0 }}
                          animate={{ height: 'auto', opacity: 1 }}
                          exit={{ height: 0, opacity: 0 }}
                          transition={{ duration: 0.18, ease: 'easeOut' }}
                          className="overflow-hidden border-t border-sidebar-border bg-white/50"
                        >
                          <div className="p-1 space-y-0.5 max-h-48 overflow-y-auto">
                            {(node.children || []).filter((f: FileNode) => f.type === 'file').map((file: FileNode) => (
                              <div
                                key={file.id}
                                className={`py-1.5 px-2 rounded cursor-pointer text-xs transition-colors ${
                                  selectedFileId === file.id
                                    ? 'bg-blue-100 text-blue-900 font-bold border-l-2 border-blue-500 shadow-sm'
                                    : 'hover:bg-blue-50/70 text-gray-600 hover:text-gray-900'
                                }`}
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleSelectStationFile(node.id, file.id);
                                  onFileSelect(file);
                                }}
                              >
                                {file.name}
                              </div>
                            ))}
                          </div>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  </div>
                );
              }
            })}
          </div>
        )}
      </SidebarMenu>
    </ScrollArea>
  );
};

export default FileTreeView;
